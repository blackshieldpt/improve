...
- `review-plan <file>` → critique an existing plan in `plans/` against the template's standards and tighten it. If you authored the plan in this same session, also have a fresh-context subagent read it cold and report ambiguities — self-critique misses gaps you mentally fill from context the executor won't have. If the plan introduces an artifact the rest of the system must accommodate, add a **premise verifier** — a fresh-context subagent attacking the fix rather than the finding; see [references/adversarial-verification.md](references/adversarial-verification.md).
