...
**Before dispatching, run `review-plan` on any plan that adds a security control, changes a public contract, or introduces a new artifact the rest of the system must accommodate** (a cookie, a header, a config field, a migration). Those are the plans whose *premise* — not whose finding — is most likely wrong, and the refutation pass in Phase 3 does not examine premises. A fresh-context reader asking "which configurations and deployment shapes must this survive?" is the cheapest place to catch a fix that is correct in the default case and broken everywhere else.
