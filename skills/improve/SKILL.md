...
- **A falsifiability bar**: a table of `delete this → that test must fail`, which the executor fills in with the *observed* failure output. A test suite that goes green proves nothing on its own; only a test seen failing for the right reason does. A row nobody can make fail is itself the finding. See the template.
...
**Before dispatching, run `review-plan` on any plan that adds a security control, changes a public contract, or introduces a new artifact the rest of the system must accommodate** (a cookie, a header, a config field, a migration). Those are the plans whose *premise* — not whose finding — is most likely wrong, and Phase 3's refutation pass attacks findings: at that point no plan exists, so there is no fix to attack. A fresh-context reader asking "which configurations and deployment shapes must this survive?" is the cheapest place to catch a fix that is correct in the default case and broken everywhere else. Give that review a **premise verifier** — the qualifying bar, prompt, and budget are defined once in [references/adversarial-verification.md](references/adversarial-verification.md). Like all verification, the verifier is skipped at `quick`; say so in the report.
...
**When plans stack** — plan N depends on plan M — record in the index that M's tests must be re-examined after N lands, not merely re-run. A guard N adds can execute *ahead* of what M asserts, leaving M's tests green while they no longer exercise the control they are named for. "Still passing" and "still testing the same thing" are different questions.
...
- `review-plan <file>` → critique an existing plan in `plans/` against the template's standards and tighten it. If you authored the plan in this same session, also have a fresh-context subagent read it cold and report ambiguities — self-critique misses gaps you mentally fill from context the executor won't have. If the plan meets the premise-verifier bar — new artifact, security control or guard on a shared path, public-contract change — add a **premise verifier**: a fresh-context subagent attacking the fix rather than the finding. The bar, prompt, and budget are defined once in [references/adversarial-verification.md](references/adversarial-verification.md).
...
- `review-merged [<base>]` → review the **combination** of executed plans as one change against `<base>` (default: the commit they were planned at). It needs a single commit containing all of them; you never build it — ask the user for an integration branch, since Hard Rule 1 forbids merging. Each `execute` review sees one plan against a clean base and is structurally blind to what only exists in the combination: plan N's new guard running *ahead* of plan M's assertion so M's test silently stops testing anything; two plans' error paths becoming distinguishable; a conflict resolution that dropped a hunk. Run it after the last `execute` and **before** integrating. See [references/closing-the-loop.md](references/closing-the-loop.md).
