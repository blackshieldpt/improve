...
- **Cryptography**: non-cryptographic RNG used for tokens, session IDs, or reset links; certificate or hostname validation disabled (`verify=False`, `InsecureSkipVerify`, custom always-true trust callbacks); ECB mode, a static or reused IV/nonce, or a key derived by plain hashing of a password; secrets and MACs compared with a non-constant-time equality; hand-rolled constructions where a vetted primitive exists; and keys with no rotation path. Cite the primitive and its call site — these are high-confidence findings that read directly off the code.
